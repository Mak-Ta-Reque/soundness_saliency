...
def main():
    ...
    print('Using ResNet50 Object detection model Fast RCNN')
...
# Step 3: Apply inference preprocessing transforms
    ...
    for idx in range(start_idx, min(end_idx, all_images.shape[0]), bs):
        ...
        # Generate output of maodel
        with torch.no_grad():
                outputs = model(images.cuda())
                print(outputs)
                output_probs = torch.nn.Softmax(dim = 1)(outputs).detach().cpu().numpy()
        output_labels = np.argmax(output_probs, axis = 1)
        dump_file = os.path.join(dump_dir, f'batch_{idx}.pkl')
        ...
        for scale, reg_l1, reg_tv, reg_ent in \
            ...
            masks_dict[config] = [masks.numpy(),output_labels, output_probs]

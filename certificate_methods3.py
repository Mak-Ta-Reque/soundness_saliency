...
class BatchMaskedModelKcert(nn.Module):
    def __init__(self, n, h, w, K=1, scale=1., bs=1, old_mask=None):
        ...
        if K == 1:
            self.weights_t = torch.zeros(1, bs, 3, int(h/scale), int(w/scale))
        else:
            self.weights_t = torch.zeros(K+1, bs, 3, int(h/scale), int(w/scale))
    ...
    def mask(self):
        ...
        if self.scale > 1:
            ...
            return masks
    def img_forward(self, model, x, K_idx=0, masks=None, use_logits=False, noise_mean=None, noise_batch=None):
...
def learn_masks_for_batch_Kcert(model, images, target_probs = None, K=1, K_bs=None, scale=1,
    ...
    for iteration in range(steps):
        ...
        for K_idx in np.random.choice(K, K_bs, replace=False):
            ...
            if obj == 'xent':
                loss += xent(pred_probs, target_labels) +  distance(images * masks, images)
            elif obj == 'ent':
